...
def load_examples(path: str) -> List[QAExample]:
    ...
    with open(path) as f:
        context = ""
        for line in f:
            tokenized_text = line[:-1].split("\t")
            if tokenized_text[1] == "문맥":
                context = tokenized_text[2]
            elif tokenized_text[2] and context:
                examples.append(QAExample(context, tokenized_text[2]))
    return examples
