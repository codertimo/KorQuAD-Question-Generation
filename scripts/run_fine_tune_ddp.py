...
def run(rank: int, world_size: int, config: QGConfig):
    ...
    korquad_train_examples = load_korquad_dataset(config.train_dataset)
    article_domain_train_examples = load_korquad_dataset(config.article_domain_train_dataset)
    train_examples = korquad_train_examples + article_domain_train_examples

    train_dataset = QGDataset(train_examples, tokenizer, config.max_sequence_length)
    train_sampler = DistributedSampler(
        train_dataset, num_replicas=world_size, rank=rank, seed=config.random_seed, shuffle=True
    )
    ...
    korquad_dev_examples = load_korquad_dataset(config.dev_dataset)
    korquad_dev_dataset = QGDataset(korquad_dev_examples, tokenizer, config.max_sequence_length, is_train=False)
    korquad_dev_dev_dataloader = DataLoader(
        korquad_dev_dataset, config.dev_batch_size, collate_fn=dynamic_padding_collate_fn
    )

    article_dev_examples = load_korquad_dataset(config.article_domain_dev_dataset)
    article_dev_dataset = QGDataset(article_dev_examples, tokenizer, config.max_sequence_length, is_train=False)
    article_dev_dev_dataloader = DataLoader(
        article_dev_dataset, config.dev_batch_size, collate_fn=dynamic_padding_collate_fn
    )
    ...
    for epoch_id in range(config.epochs):
        for step_index, batch_data in tqdm(
            ...
            input_ids, attention_mask, labels = tuple(value.to(rank) for value in batch_data)
            ...
            if rank == 0 and global_step % config.validation_interval == 0:
                logger.info("============KorQuAD dev set==============")
                _validate(model, korquad_dev_dev_dataloader, rank, logger, global_step)
                logger.info("============Article QA dev set==============")
                _validate(model, article_dev_dev_dataloader, rank, logger, global_step)
...
def _validate(
    model: Union[GPT2LMHeadModel, DistributedDataParallel],
